    ...
    if use_merging:
        ...
        if profile:
            ...
            if hasattr(result, 'additional_info') and result.additional_info:
                profiling_info['clustering_details'] = result.additional_info
...
def visualise_pair_best(df, feat1, feat2, result: PairAnalysisResult):
    ...
    df_with_values = pd.DataFrame({feat1: result.X.flatten(), feat2: result.y, 'bin': result.clusters_x.bin_assignments})
    # whis=0 shows all points outside 25-75% as outliers (circles).  Can't seem to actually disable outlier detection, which we don't need as that's done separately.
    sns.boxplot(x='bin', y=feat2, data=df_with_values, ax=axes[2], palette=bin_colors, order=bins_to_show, whis=0)
    axes[2].set_xlabel('Bin')
    axes[2].set_ylabel(feat2)
    axes[2].set_title(f'ANOVA F={result.clusters_x.anova.f_value:.2f}, p={result.clusters_x.anova.p_value:.4f} (excluded={len(result.clusters_x.anova.excluded_bins)})')
...
def print_pair_analysis(pair_result: PairAnalysisResult):
    ...
    a = pair_result.clusters_x.anova
    print(f"Binning:")
    print(f" Method: {pair_result.clusters_x.method}")
    print(f" ANOVA: F={a.f_value:.3f}, p={a.p_value:.3f}")
    for lbl, mean in a.bin_means.items():
        std = a.bin_stds[lbl]
        print(f"  Bin {lbl}: mean={mean:.3f}, std={std:.3f} size={len(pair_result.clusters_x.bin_contents[lbl])} {pair_result.clusters_x.anova.excluded_bins.get(lbl, '')}")
